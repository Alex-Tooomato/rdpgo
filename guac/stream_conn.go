...
type Stream struct {
	conn         net.Conn
	ConnectionID string // ConnectionID is the ID Guacamole gives and can be used to reconnect or share sessions
	parseStart   int    // if more than a single instruction is read, the rest are buffered here
	buffer       []byte
	reset        []byte
